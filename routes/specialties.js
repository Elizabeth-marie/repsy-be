...
const validateUserID = (req, res, next) => {
  knex('specialties').where('id', req.params.id).then(([data]) => {
    if (!data) {
...
router.get('/', (req, res, next) => {
  knex('specialties').then(data => res.status(200).json(data)).catch(err => next(err))
})

/* GET single users record */
router.get('/:id', validateUserID, (req, res, next) => {
  knex('specialties').where('id', req.params.id).then(([data]) => res.status(200).json(data)).catch(err => next(err))
})
//
// /* POST new users record */
// http POST http://localhost:3000/specialties name="test"
router.post('/', validatePostBody, (req, res, next) => {
  const {id, name} = req.body

  knex('specialties').insert({id, name}).returning('*').then(([data]) => res.status(201).json(data)).catch(err => next(err))
})
//
/* PATCH specified users record */
//http PATCH http://localhost:3000/specialties/14 name="Dietetics"
router.patch('/:id', validateUserID, buildPatchReq, (req, res, next) => {
  const {patchReq} = req

  knex('specialties').where('id', req.params.id).first().update(patchReq).returning('*').then(([data]) => {
...
router.delete('/:id', validateUserID, (req, res, next) => {
  knex('specialties').where('id', req.params.id).first().del().returning('*').then(([data]) => {
