...
const validateUserID = (req, res, next) => {
  knex('doctors_reps').where('id', req.params.id).then(([data]) => {
    if (!data) {
      return res.status(400).json({
        error: {
          message: `doctors_reps ID ${req.params.id} not found`
        }
      })
    }
    next()
...
const buildPatchReq = (req, res, next) => {
  const patchSchema = Joi.object().keys({
    reps_id: Joi.number().integer(),
    doctors_id: Joi.number().integer()
  })
  ...
  const allowedPatchKeys = ['reps_id', 'doctors_id']
...
/* GET all doctors_reps record */
router.get('/', (req, res, next) => {
  knex('doctors_reps').then(data => res.status(200).json(data)).catch(err => next(err))
})

/* GET single doctors_reps record */
router.get('/:id', validateUserID, (req, res, next) => {
  knex('doctors_reps').where('id', req.params.id).then(([data]) => res.status(200).json(data)).catch(err => next(err))
})

/* POST new doctors_reps record */
// http POST http://localhost:3000/doctors_reps doctors_id=3 reps_id=1
router.post('/', validatePostBody, (req, res, next) => {
  const {id, reps_id, doctors_id} = req.body

  knex('doctors_reps').insert({id, reps_id, doctors_id}).returning('*').then(([data]) => res.status(201).json(data)).catch(err => next(err))
})

/* PATCH specified doctors_reps record */
router.patch('/:id', validateUserID, buildPatchReq, (req, res, next) => {
  const {patchReq} = req

  knex('doctors_reps').where('id', req.params.id).first().update(patchReq).returning('*').then(([data]) => {
    res.status(200).json(data)
  }).catch(err => next(err))
})

/* DELETE specified doctors_reps record */
router.delete('/:id', validateUserID, (req, res, next) => {
  knex('doctors_reps').where('id', req.params.id).first().del().returning('*').then(([data]) => {
